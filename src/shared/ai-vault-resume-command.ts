...
export function buildAiVaultResumeCommand(args: {
  ...
  return buildAiVaultResumeShellCommand({
    resumeCommand: buildAgentResumeInvocation(agent, baseCommand, sessionArg),
    cwd,
    platform,
    codexHome,
    shell
...
export function realHomeCodexResumeEnvDeletion(
  ...
  if (!isCodexRuntimeAgentType(session.agent) || session.codexHome !== null) {
    return {}
  }
...
function quoteShellArg(value: string, platform: NodeJS.Platform): string {
  return platform === 'win32' ? quoteWindowsCmdArg(value) : `'${value.replace(/'/g, `'\\''`)}'`
}
