...
const AI_VAULT_AGENT_SET: ReadonlySet<string> = new Set(AI_VAULT_AGENTS)

export function toAiVaultAgent(agent: string | null | undefined): AiVaultAgent | null {
  if (isCodexRuntimeAgentType(agent)) {
    return 'codex'
  }
  return typeof agent === 'string' && AI_VAULT_AGENT_SET.has(agent) ? (agent as AiVaultAgent) : null
}

export function aiVaultAgentMatchesRuntimeAgent(
  session: Pick<AiVaultSession, 'agent'>,
  runtimeAgent: string | null | undefined
): boolean {
  return toAiVaultAgent(runtimeAgent) === session.agent
}
